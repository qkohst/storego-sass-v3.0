...
@endpush
@section('content')
    <section class="top-product my-cart-section mt-5 pt-5">
        <div class="container">
                    ...
                    <div class="col-xl-3 col-lg-4 col-sm-6 product-box wishlist_{{$product['product_id']}}">
                        <div class="card card-product">
                            ...
                            <div class="card-body mt-3">
                                    ...
                                    @if($store['enable_rating'] == 'on')
                                        @for($i =1;$i<=5;$i++)
                                            @php
                                                ...
                                                $color = '';
                                                $newVal1 = ($i-0.5);
                                                if(\App\Product::getRatingById($product['product_id']) < $i && \App\Product::getRatingById($product['product_id']) >= $newVal1)
                                                {
                                                    $icon = 'fa-star-half-alt';
                                                }
                                                if(\App\Product::getRatingById($product['product_id']) >= $newVal1)
                                                {
                                                    $color = 'text-warning';
                                                }
                                            @endphp
                                ...
                                @else
                                    <div class="product-price mt-3">
                                        <span class="card-price t-black15">{{\App\Utility::priceFormat($product['price'])}}</span>
                                    </div>
                                @endif
                                <p class="text-sm t-black15">
                                    {{__('Category')}}: {{\App\Product::getCategoryById($product['product_id'])}}
                                </p>
                            </div>
